...
def translate_text(transl_textbox, sysprom, rawtext, histories, stream, ask=False):
    ...
    if not connected:
       root.after(0, create_error_window, "API key not connected")
       return

    prompt_text = sysprom.get(1.0, tk.END).strip()
    text_to_translate = rawtext.get(1.0, tk.END).strip()
    
    messages = [{"role": "system", "content": f"{prompt_text}"}]
    if dictionary:
        messages.append({"role": "system", "content":f"{dictionary}"})
    if ask:
        for message in history[-(context_size//2):]:
            messages.append(message)
    for message in histories[-(context_size//2):]:
        messages.append(message)
    messages.append({"role": "user", "content": f"{text_to_translate}"})
    ...
    if stream:
        ...
        for chunk in response:
            ...
            if "upstream error" in str(chunk_message):
                root.after(0, create_error_window, chunk_message)
                return    
            elif chunk_message:
                update_display(transl_textbox, chunk_message)
        ...
        update_display(transl_textbox, translated_text)
        
    translated_dict[text_to_translate] = translated_text
    histories.append({"role": "user", "content": text_to_translate})
...
def set_parameters():
    global max_response_length, frequency_penalty, presence_penalty, temperature, context_size
    ...
    def save_parameters():
        ...
        frequency_penalty = float(frequency_penalty_var.get()) if frequency_penalty_var.get() else frequency_penalty
        presence_penalty = float(presence_penalty_var.get()) if presence_penalty_var.get() else presence_penalty
    ...
    frequency_penalty_label = ttk.Label(params_window, text="Frequency Penalty:")
    frequency_penalty_label.pack(side=tk.TOP, pady=(20, 5))
    frequency_penalty_entry = ttk.Entry(params_window, textvariable=frequency_penalty_var, width=15)
    frequency_penalty_entry.pack(side=tk.TOP, padx=10)

    presence_penalty_var = tk.StringVar(value=presence_penalty)
    presence_penalty_label = ttk.Label(params_window, text="Presence Penalty:")
    presence_penalty_label.pack(side=tk.TOP, pady=(20, 5))
    presence_penalty_entry = ttk.Entry(params_window, textvariable=presence_penalty_var, width=15)
...
def set_openai_api():
    global model, connected
    api_window = tk.Toplevel()
    api_window.transient(root)
    api_window.title("OpenAI API")
    api_window.geometry("350x200")
    ...
    api_base_label = ttk.Label(api_window, text="API Base:")
    api_base_label.pack(side=tk.TOP, pady=5)
    api_base_combobox = ttk.Combobox(api_window, textvariable=api_base, width=50, values=old_api_bases)
    api_base_combobox.insert(0, openai.api_base)
    api_base_combobox.pack(side=tk.TOP, padx=10)

    api_key = tk.StringVar()
    api_key_label = ttk.Label(api_window, text="API Key:")
    api_key_label.pack(side=tk.TOP, pady=5)
    api_key_combobox = ttk.Combobox(api_window, textvariable=api_key, width=50, values=old_api_keys)
    api_key_combobox.insert(0, openai.api_key)
    api_key_combobox.pack(side=tk.TOP, padx=10)
    ...
    model_options = ["gpt-3.5", "gpt-3.5-turbo", "gpt-3.5-turbo-0301", "gpt-4", "gpt-4-0314", "gpt-4-32k"]
    ...
    def check_api_key():
        connected = False
        try:
            response = requests.get(f"{api_base.get()}/models", headers={"Authorization": f"Bearer {api_key.get()}"})
            if response.status_code == 200:
                ...
                connected_label.config(text="Invalid API key", foreground="red")
        except Exception as e:
            if requests.exceptions.ConnectionError:
                connected_label.config(text=f"Connection Error.", foreground="red")
            else:
                connected_label.config(text=f"Error: {e}", foreground="red")
        return connected

    def save_api_settings():
        global model, connected
        if connected and api_base.get() != openai.api_base or api_key.get() != openai.api_key:
            connected = check_api_key()
        else:
            connected = connected_var.get()
        openai.api_base = api_base.get()
        openai.api_key = api_key.get()
        model = model_var.get()
        
        if api_base.get() not in old_api_bases:
            old_api_bases.append(api_base.get())
            save_settings("old_api_bases", ",".join(old_api_bases))
        if api_key.get() not in old_api_keys:
            old_api_keys.append(api_key.get())
            save_settings("old_api_keys", ",".join(old_api_keys))
    ...
    connected_var = tk.BooleanVar(value=connected)
    connected_label = ttk.Label(api_window, text="Connected" if connected else "Not Connected", foreground="green" if connected else "red")
    connected_label.place(relx=0.55, rely=0.7, anchor="s")
    connect_button = ttk.Button(api_window, text="Connect", command=lambda: connected_var.set(check_api_key()))
    connect_button.place(relx=0.55, rely=0.9, anchor="s")
    
    save_button = ttk.Button(api_window, text="Save", command=save_api_settings)
    save_button.place(relx=0.95, rely=0.7, anchor="se")
    cancel_button = ttk.Button(api_window, text="Cancel", command=lambda: api_window.destroy())
    cancel_button.place(relx=0.95, rely=0.9, anchor="se")
...
def check_api_key():
    connected = False
    try:
        response = requests.get(f"{openai.api_base}/models", headers={"Authorization": f"Bearer {openai.api_key}"})
        if response.status_code == 200:
            connected = True
    except Exception as e:
        if requests.exceptions.ConnectionError:
            root.after(0, create_error_window, f"Connection Error.")
            return
        root.after(0, create_error_window, f"{e}")
        return
    return connected
...
def history_window():
    ...
    def save_history(history_table):
        ...
        for item in history_table.get_children():
            ...
            history.append({"role": "user", "content": untranslated_text})
...
old_api_bases = settings_data.get("old_api_bases", "").split(",")
old_api_keys = settings_data.get("old_api_keys", "").split(",")
...
frequency_penalty = float(settings_data.get("frequency_penalty", 0))
presence_penalty = float(settings_data.get("presence_penalty", 0))
...
if openai.api_base and openai.api_base != "https://api.openai.com/v1":
    connected = check_api_key()
...
openai_menu = Menu(menu_bar, tearoff=0)
menu_bar.add_cascade(label="OpenAI", menu=openai_menu)
openai_menu.add_command(label="API", command=set_openai_api)
openai_menu.add_command(label="Parameters", command=set_parameters)
openai_menu.add_checkbutton(label="Streaming", variable=streaming_var, command=toggle_streaming, accelerator="Alt-S")
openai_menu.add_command(label="History", command=history_window)
openai_menu.add_command(label="Dictionary", command=dictwin)
